...
Game::Game(int window_width, int window_height, SDL_Window* window)
{
	...
	this->playStage = new PlayStage();
	this->looseStage = new LooseStage();
...
{
	if (number == 0) { currentStage = introStage; }
	if (number == 1) { restart(); currentStage = playStage; }
	if (number == 2) { currentStage = looseStage; }
	if (number == 3) { currentStage = winStage; }
}
...
void Game::render(void)
{
	...
	for (int l = 0; l < 5; l++) {
		map->drawMap(&framebuffer, l, time, tileset, camera);
	}*/
